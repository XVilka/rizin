...
static void printkey_cb(void *user, const char *k, const char *v) {
	rz_cons_println(k);
}
...
static void print_typelist_r_cb(void *p, const char *k, const char *v) {
...
static bool print_type_c(RzCore *core, const char *ctype) {
	...
	if (name && type) {
		...
		if (rz_str_startswith(type, "struct")) {
			...
			rz_core_types_typedef_print_c(TDB, name);
		} else if (rz_str_startswith(type, "func")) {
			rz_core_types_function_print(core, name, RZ_OUTPUT_MODE_STANDARD, NULL);
		}
		return true;
	}
	return false;
...
static void type_list_c_all(RzCore *core) {
	...
	rz_core_types_typedef_print_c(TDB, NULL);
...
static void cmd_type_noreturn(RzCore *core, const char *input) {
	...
	case '-': // "tn-"
		if (input[1] == '*') {
			RzList *noretl = rz_core_types_function_noreturn(core);
			RzListIter *iter;
			char *name;
...
static bool nofilterfn(void *user, const char *k, const char *v) {
	return true;
}

/*
RZ_API void rz_print_sdb_filter(Sdb *db,
		bool (*filterfn)(Sdb *db, void *user, const char *k, const char *v),
		void (*printfn)(PJ *pj, void *user, cosnt char *k, const char *v),
...
static void typesList(RzCore *core, int mode) {
	...
	case 1:
	case '*':
		rz_print_sdb_filter(TDB, nofilterfn, print_typelist_r_cb, false, NULL, NULL);
		break;
	case 'j':
		rz_print_sdb_filter(TDB, stdiftype, print_typelist_json_cb, true, PJ, NULL);
		break;
	default:
		rz_print_sdb_filter(TDB, stdiftype, printkey_cb, true, PJ, NULL);
		break;
	}
}
...
RZ_IPI int rz_cmd_type(void *data, const char *input) {
	...
	case 'f': // "tf"
		...
		case 'k': // "tfk"
			if (input[2] == ' ') {
				...
				rz_core_types_function_print(core, name, RZ_OUTPUT_MODE_SDB, NULL);
			} else {
		...
		case 'j': // "tfj"
			if (input[2] == ' ') {
				...
				PJ *pj = rz_core_pj_new(core);
				rz_core_types_function_print(core, name, RZ_OUTPUT_MODE_JSON, pj);
				pj_end(pj);
				rz_cons_println(pj_string(pj));
				pj_free(pj);
			} else {
	...
	case 't': { // "tt"
		...
		if (input[1] == 'c') { // "ttc"
			rz_core_types_typedef_print_c(TDB, input + 2);
			break;
		}
		if (input[1] == '?') { // "tt?"
...
RZ_IPI RzCmdStatus rz_type_list_function_handler(RzCore *core, int argc, const char **argv, RzOutputMode mode) {
	...
	if (function) {
		PJ *pj = (mode == RZ_OUTPUT_MODE_JSON) ? rz_core_pj_new(core) : NULL;
		rz_core_types_function_print(core, function, mode, pj);
		if (mode == RZ_OUTPUT_MODE_JSON) {
			rz_cons_println(pj_string(pj));
			pj_free(pj);
		}
...
RZ_IPI RzCmdStatus rz_type_link_handler(RzCore *core, int argc, const char **argv, RzOutputMode mode) {
	...
	const char *addr = argc > 2 ? argv[2] : NULL;
	if (name) {
		if (addr) {
			rz_core_types_link(core, name, rz_num_math(core->num, addr));
		} else {
			rz_core_types_link(core, name, core->offset);
		}
	} else {
...
RZ_IPI RzCmdStatus rz_type_typedef_c_handler(RzCore *core, int argc, const char **argv) {
	const char *typename = argc > 1 ? argv[1] : NULL;
	if (!typename) {
		rz_core_types_typedef_print_c(core, NULL);
		return RZ_CMD_STATUS_OK;
	}
	rz_core_types_typedef_print_c(core, typename);
	return RZ_CMD_STATUS_OK;
}
