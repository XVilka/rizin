...
RZ_IPI void rz_core_types_enum_print_all(RzCore *core, RzOutputMode mode) {
	...
	rz_list_foreach (enumlist, it, e) {
		types_enum_print(core, e, mode, pj);
	}
...
RZ_IPI void rz_types_struct_print_c(Sdb *TDB, const char *name, bool multiline) {
	SdbList *l = sdb_foreach_list_filter(TDB, sdb_if_struct_cb, true);
	rz_types_structured_print_c(TDB, l, name, multiline);
	ls_free(l);
}
...
RZ_API void rz_core_types_typedef_print_c(Sdb *TDB, const char *typedef_name) {
	char *name = NULL;
	SdbKv *kv;
	SdbListIter *iter;
	SdbList *l = sdb_foreach_list(TDB, true);
...
RZ_IPI void rz_core_types_function_print(RzCore *core, const char *function, RzOutputMode mode, PJ *pj) {
